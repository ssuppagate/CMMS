...
router.post("/tbl_admin2", (req, res, next) => {
  ...
  // res.send('hello')
  connect.query(
    "INSERT INTO tbl_admin (admin_name,admin_designation,admin_email,admin_password,admin_phone,admin_address,created_timestamp,updated_timestamp) VALUES(?,?,?,?,?,?,now(),now())",
    [
      admin_name,
      admin_designation,
      admin_email,
...
router.put("/update/:admin_id", (req, res, next) => {
  ...
  connect.query(
    "UPDATE tbl_admin SET admin_name=?,admin_email=?,admin_password=?,admin_phone=?,admin_address=?,admin_designation=?,created_timestamp=now(),updated_timestamp=now() WHERE admin_id = ?",
    [
      admin_name,
      admin_email,
      admin_password,
...
//update dataEmployee
router.put("/update/:employee_id", (req, res, next) => {
  const employee_name = req.body.employee_name;
  //const admin_designation = req.body.admin_designation;
  const employee_email = req.body.employee_email;
  //const admin_password = req.body.admin_password;
  ...
  console.log("edit", req.body);
  connect.query(
    "UPDATE CMMS.tbl_employee SET employee_name=?,employee_email=?,employee_address=?,employee_phone=?,created_timestamp=now(),updated_timestamp=now() WHERE employee_id = ?",
    [
      employee_name,
      employee_email,
      employee_address,
...
//update dataEmployee
router.put("/update/:device_id", (req, res, next) => {
  const device_name = req.body.device_name;
  const device_warranty = req.body.device_warranty;
  const device_producer = req.body.device_producer;
  const device_cost = req.body.device_cost;
  ...
  console.log("edit", req.body);
  connect.query(
    "UPDATE tbl_device SET device_name=?,device_warranty=?,device_producer=?,device_cost=?,device_image=?,device_note=?,device_status=?,device_model=?,device_serial=?,device_asset_tag=?,created_timestamp=now(),updated_timestamp=now() WHERE device_id = ?",
    [
      device_name,
      device_warranty,
      device_producer,
...
router.post("/tbl_list_repair", (req, res, next) => {
  ...
  // res.send('hello')
  connect.query(
    "INSERT INTO tbl_list_repair (case_detail,created_timestamp,updated_timestamp) VALUES(?,now(),now())",
...
router.get("/getstatus/:id", async (req, res) => {
  ...
  try {
    ...
    connect.query(
      `SELECT * FROM boi_it_smt WHERE id= '\t${id}\t'`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
...
router.put("/put/repair/:id", (req, res, next) => {
  const lineNotify = require("line-notify-nodejs")(
    "G9lzeks0DzzT4BmXglynx3EtmyIkygN3NekiwUI1zuL"
  );
...
router.put("/update/status/:id", (req, res, next) => {
  ...
  connect.query(
    "UPDATE tbl_repair SET status=?,admin_id=?,priority=? WHERE id = ?",
    [status, admin_id, priority, id],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
router.put("/update/statusComplete/:id", (req, res, next) => {
  ...
  console.log("edit55", req.body);
  connect.query(
    "UPDATE tbl_repair SET status=? WHERE id = ?",
    [status, id],
    (err, result) => {
      if (err) {
        console.log(err);
...
router.get("/get/status/device/:device_id", (req, res, next) => {
  ...
  connect.query(
    "SELECT * FROM tbl_repair WHERE device_id = ? ",
    device_id,
    (err, rows) => {
      if (err) {
        res.send(err);
